...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Response<?>> handleAuthorizationException(AuthorizationException ex) {
        return ResponseUtil.prepareErrorResponse(HttpStatus.UNAUTHORIZED, ex.getMessage());
    }

    @ExceptionHandler(ValidationException.class)
    public ResponseEntity<Response<?>> handleValidationException(ValidationException ex) {
        return ResponseUtil.prepareErrorResponse(HttpStatus.BAD_REQUEST, ex.getMessage());
    }

    @ExceptionHandler(NoDataFoundException.class)
    public ResponseEntity<Response<?>> handleNoDataFoundException(NoDataFoundException ex) {
        return ResponseUtil.prepareErrorResponse(HttpStatus.NOT_FOUND, ex.getMessage());
    }

    @ExceptionHandler(PlatformException.class)
    public ResponseEntity<Response<?>> handlePlatformException(PlatformException ex) {
        return ResponseUtil.prepareErrorResponse(HttpStatus.BAD_GATEWAY, ex.getMessage());
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<Response<?>> handleGenericException(Exception ex) {
        return ResponseUtil.prepareErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR, ex.getMessage());
    }
}
